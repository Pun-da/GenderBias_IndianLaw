...
def main(args):
    ...
    print("female : ", female_attrs)
    print("male : ", male_attrs)

    # sys.exit()


    cluster2word = load_clusters(args.clusters, wv)
...
def cosine_means_difference(wv, word, male_attrs, female_attrs):
    ...
    # print(f"male  mean for {word} : {male_mean}")
    # print(f"female  mean for {word} : {female_mean}")

    return male_mean - female_mean

def cosine_mean(wv, word, attrs):
    
    attrs_vector = np.array([wv[attr] for attr in attrs])
    # print("wv[attrs] : " , attrs_vector.shape)
    word_vector = wv[word]
    # word_vector = word_vector.reshape(1, -1)
    # print("wv[wprd] : ", word_vector.shape)
    similarities  = wv.cosine_similarities(word_vector, attrs_vector)
    # print(f"dskkfkds {similarities}")
    return similarities.mean()

def get_cluster_description(wv, cluster_words):
    cluster_centroid = wv[cluster_words].sum(axis=0) / len(cluster_words)
    most_similars = wv.most_similar(positive=[cluster_centroid], topn=10)
...
def load_clusters(cluster_file, wv):
    cluster2word = {}
    with open(cluster_file, 'r') as fclus:
        for line in fclus:
            fields = line.strip().split('\t')
            if len(fields) != 2:
                ...
                word = fields[0]
                cluster = int(fields[1])
            if word not in wv.index_to_key:
                continue
            cluster_rec = cluster2word.get(cluster, None)
            if cluster_rec is None:
                cluster_rec = []
...
def filter_words(wv, words):
    #return only words that are available in the vocab
    final_words = [word for word in words if word in wv.index_to_key]
    return final_words
