...
def weat_rand_test(wv, m_words, f_words, m_attrs, f_attrs, iterations):
    ...
    print(f"u_words inside weat_rand_test : {u_words}")
    ...
    for _ in range(runs):
        ...
        if permutation not in seen:
            ...
            temp = test_statistic(wv, m_hat, f_hat, m_attrs, f_attrs)
            # print(temp)
            # print("----------------------")
            if temp > original:
                r += 1
...
def test_sums(wv, m_targets, f_targets, m_attrs, f_attrs):
    m_sum = 0.0
    f_sum = 0.0
    # print(f"...{m_targets}, {m_attrs}, {f_targets}, {f_attrs}...")
    for t in m_targets:
        temp = ga.cosine_means_difference(wv, t, m_attrs, f_attrs)
        # print(f"for {t}: {temp}")
        m_sum += temp
    for t in f_targets:
